...
-----------------------------------------------------------------------------
ボールがブロックにぶつかる時正確な方向に向かうように注意しました。
又、エディターでは好きな色と大きさで、好きな位置にブロックを作るようにしました。
...
-----------------------------------------------------------------------------
ブロックをｘ型で分けてあたる時のボールの中心の座標と移動方向で次の方向を
決めるようにしましたがプレームのせいでボールが早いとき、又は角の方に当たる時に
変な方向に行くバーグが多いでした。
当たる時に便だけ当たるようにボールを進行方向と反対側に戻して進行方向を
決めるようにして解決しました。
...
-----------------------------------------------------------------------------
授業で作ったもので色々教えてもらいましたが、ブールの衝突判定だけは自分の
方式で何回もやり直しながら完成しました。
エディターで作ったマップをすぐプレイすることができませんが適用することは
できるので4番目のステージは実際にエディターで作ったものです。
一回だけでは壊されないブロックも具現しました。
